...
R f 0 1 - Ja Wednesday>=30 1w 2s -
  ...
  for (std::size_t i = 0; i < result.__rules.size(); ++i) {
    ...
    r = std::get<std::chrono::__constrained_weekday>(result.__rules[i].second[0].__on);
    ...
    r = std::get<std::chrono::__constrained_weekday>(result.__rules[i].second[1].__on);
    assert(r.__weekday == std::chrono::Monday);
    assert(r.__comparison == std::chrono::__constrained_weekday::__le);
    assert(r.__day == std::chrono::day(1));

    r = std::get<std::chrono::__constrained_weekday>(result.__rules[i].second[2].__on);
    assert(r.__weekday == std::chrono::Saturday);
    assert(r.__comparison == std::chrono::__constrained_weekday::__ge);
    assert(r.__day == std::chrono::day(31));

    r = std::get<std::chrono::__constrained_weekday>(result.__rules[i].second[3].__on);
    assert(r.__weekday == std::chrono::Sunday);
    assert(r.__comparison == std::chrono::__constrained_weekday::__le);
    assert(r.__day == std::chrono::day(31));

    r = std::get<std::chrono::__constrained_weekday>(result.__rules[i].second[4].__on);
    assert(r.__weekday == std::chrono::Thursday);
    assert(r.__comparison == std::chrono::__constrained_weekday::__ge);
    assert(r.__day == std::chrono::day(10));

    r = std::get<std::chrono::__constrained_weekday>(result.__rules[i].second[5].__on);
    assert(r.__weekday == std::chrono::Tuesday);
    assert(r.__comparison == std::chrono::__constrained_weekday::__le);
    assert(r.__day == std::chrono::day(20));

    r = std::get<std::chrono::__constrained_weekday>(result.__rules[i].second[6].__on);
...
R a 0 1 - Ja Su>=31 - 2s -
  ...
  assert(result.__rules[0].second[0].__at_time == std::chrono::hours(2));
  assert(result.__rules[0].second[0].__at_clock == std::chrono::__time_zone_rule::__clock::__local);

  assert(result.__rules[0].second[1].__at_time == std::chrono::hours(2));
  assert(result.__rules[0].second[1].__at_clock == std::chrono::__time_zone_rule::__clock::__standard);

  assert(result.__rules[0].second[2].__at_time ==
         std::chrono::hours(1) + std::chrono::minutes(28) + std::chrono::seconds(14));
  assert(result.__rules[0].second[2].__at_clock == std::chrono::__time_zone_rule::__clock::__universal);

  assert(result.__rules[0].second[3].__at_time == std::chrono::minutes(19) + std::chrono::seconds(32));
  assert(result.__rules[0].second[3].__at_clock == std::chrono::__time_zone_rule::__clock::__universal);

  assert(result.__rules[0].second[4].__at_time == std::chrono::hours(12));
  assert(result.__rules[0].second[4].__at_clock == std::chrono::__time_zone_rule::__clock::__universal);

  assert(result.__rules[0].second[5].__at_time == std::chrono::hours(15));
  assert(result.__rules[0].second[5].__at_clock == std::chrono::__time_zone_rule::__clock::__local);

  assert(result.__rules[0].second[6].__at_time == std::chrono::hours(24));
  assert(result.__rules[0].second[6].__at_clock == std::chrono::__time_zone_rule::__clock::__local);

  assert(result.__rules[0].second[7].__at_time == std::chrono::hours(260));
  assert(result.__rules[0].second[7].__at_clock == std::chrono::__time_zone_rule::__clock::__local);

  assert(result.__rules[0].second[8].__at_time == -(std::chrono::hours(2) + std::chrono::minutes(30)));
  assert(result.__rules[0].second[8].__at_clock == std::chrono::__time_zone_rule::__clock::__local);

  assert(result.__rules[0].second[9].__at_time == std::chrono::hours(0)); // The man page expresses it in hours
  assert(result.__rules[0].second[9].__at_clock == std::chrono::__time_zone_rule::__clock::__local);
}
