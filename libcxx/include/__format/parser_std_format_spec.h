...
template <class _CharT>
concept __utf16 = (same_as<_CharT, wchar_t> && sizeof(wchar_t) == 2) ||
                  same_as<_CharT, char16_t>;

/** Helper concept for an UTF-32 character type. */
template <class _CharT>
concept __utf32 = (same_as<_CharT, wchar_t> && sizeof(wchar_t) == 4) ||
                  same_as<_CharT, char32_t>;

/** Helper concept for an UTF-16 or UTF-32 character type. */
template <class _CharT>
concept __utf16_32 = __utf16<_CharT> || __utf32<_CharT>;
...
__get_string_alignment(const _CharT* __first, const _CharT* __last,
  ...
  const _CharT* __size =
      __first +
      (__precision == -1 ? __length : _VSTD::min(__length, __precision));
  return {__first + __size, __size, __size < __width};
}
