...
_LIBCPP_HIDE_FROM_ABI __float_result __format_buffer(__float_buffer<_Fp>& __buffer, _Tp __value, bool __negative,
                                                     bool __has_precision, __format_spec::__sign __sign,
                                                     __format_spec::__type __type) {
  char* __first = __insert_sign(__buffer.begin(), __negative, __sign);
  switch (__type) {
  ...
  case __format_spec::__type::__float_hexadecimal_lower_case:
    return __formatter::__format_buffer_hexadecimal_lower_case(__buffer, __value,
                                                               __has_precision ? __buffer.__precision() : -1, __first);
...
_LIBCPP_HIDE_FROM_ABI _OutIt __format_locale_specific_form(_OutIt __out_it, const __float_buffer<_Fp>& __buffer,
  ...
  ptrdiff_t __size = __result.__last - __buffer.begin() + // Formatted string
  ...
  bool __zero_padding = __parser.__alignment_ == __format_spec::__alignment::__default;
  if (__size < __parser.__width_) {
    if (__zero_padding) {
      __parser.__alignment_ = __format_spec::__alignment::__right;
      __parser.__fill_ = _CharT('0');
    }
...
_LIBCPP_HIDE_FROM_ABI _OutIt __format_floating_point_non_finite(_OutIt __out_it,
                                                                __format_spec::__parser<_CharT> __parser,
                                                                bool __negative, bool __isnan) {
  char __buffer[4];
  char* __last = __formatter::__insert_sign(__buffer, __negative, __parser.__sign_);
  ...
  __last =
      _VSTD::copy_n(&("infnanINFNAN"[6 * (__parser.__type_ == __format_spec::__type::__float_hexadecimal_upper_case ||
                                          __parser.__type_ == __format_spec::__type::__scientific_upper_case ||
                                          __parser.__type_ == __format_spec::__type::__fixed_upper_case ||
                                          __parser.__type_ == __format_spec::__type::__general_upper_case) +
                                     3 * __isnan]),
                    3, __last);
  ...
  return __formatter::__write(__buffer, __last, _VSTD::move(__out_it), __parser);
}

template <floating_point _Tp, class _CharT>
_LIBCPP_HIDE_FROM_ABI auto __format_floating_point(_Tp __value, auto& __ctx, __format_spec::__parser<_CharT> __parser)
    -> decltype(__ctx.out()) {

  bool __negative = _VSTD::signbit(__value);

  if (!_VSTD::isfinite(__value)) [[unlikely]]
    return __formatter::__format_floating_point_non_finite(__ctx.out(), __parser, __negative, _VSTD::isnan(__value));
  ...
  __float_buffer<_Fp> __buffer(__parser.__precision_);
  __float_result __result = __formatter::__format_buffer(__buffer, __value, __negative, (__parser.__has_precision()),
                                                         __parser.__sign_, __parser.__type_);

  if (__parser.__alternate_form_ && __result.__radix_point == __result.__last) {
    *__result.__last++ = '.';
...
#    ifndef _LIBCPP_HAS_NO_LOCALIZATION
  if (__parser.__locale_specific_form_)
    return __formatter::__format_locale_specific_form(__ctx.out(), __buffer, __result, __ctx.locale(), __parser);
#    endif

  ptrdiff_t __size = __result.__last - __buffer.begin();
  int __num_trailing_zeros = __buffer.__num_trailing_zeros();
  if (__size + __num_trailing_zeros >= __parser.__width_) {
  ...
  if (__num_trailing_zeros)
    return __formatter::__write_using_trailing_zeros(__first, __result.__last, _VSTD::move(__out_it), __parser, __size,
                                                     __result.__exponent, __num_trailing_zeros);

  return __formatter::__write(__first, __result.__last, _VSTD::move(__out_it), __parser, __size);
}
