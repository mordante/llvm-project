...
class tester {
  ...
  constexpr void validate(auto it) const noexcept {
    assert(it == end());
    auto result = __get_string_alignment(begin(), end(), 1'000'000, 1'000'000);
    assert(result.__last == end() &&
           static_cast<size_t>(result.__size) ==
               __detail::__estimate_column_width(begin(), end(), -1).__width &&
           result.__align);
  }
  ...
  void test(benchmark::State& state) const {
    for (auto _ : state)
      benchmark::DoNotOptimize(
          __get_string_alignment(begin(), end(), 1'000'000, 1'000'000));
